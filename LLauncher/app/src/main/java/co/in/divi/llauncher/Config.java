package co.in.divi.llauncher;

/**
 * Created by indra83 on 9/29/15.
 */
public class Config {

    public static final String LAUNCH_ACTION = "co.in.divi.llauncher.LAUNCH_APP";

    public static final int LAUNCH_DIVI_TIMER = 4000;

    public static final String DIVI_ACCESSIBILITY_SERVICE = "co.in.divi/co.in.divi.apps.TopActivityMonitorService";

    public static final String APP_UPDATE_URL = "https://s3.ap-south-1.amazonaws.com/divi-launcher/demo/divi_app_update_l.json";
}
